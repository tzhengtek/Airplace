...
const logger = createLogger({ project: 'command-snapshot' });
...
functions.http('commandSnapshot', async (req, res) => {
...
async function processCommand(data) {
    ...
    try {
        ...
        const bucketName = "serverless-epitech-snapshots";
        const objectPath = "snapshot-schedule.png";
        ...
        const body = { content: `✅ Airplace snapshot retrieved.` };
        if (files.length > 0) body.files = files;

        await editOriginalMessage(cachedAppId, data.interactionToken, body);
        logger({ severity: Severity.INFO, message: 'Snapshot message sent', command: data.command, interactionToken: data.interactionToken, files: files.length });
        return;
    } catch (err) {
